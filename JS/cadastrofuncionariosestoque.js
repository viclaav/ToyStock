...
document.getElementById('form').addEventListener('submit', function(e) {
    ...
    if (funcionarioId) {
        // Atualização
        database.ref('FuncionariosEstoque/' + funcionarioId).update(funcionario)
            .then(() => {
        ...
        database.ref('FuncionariosEstoque/' + cpf).set(funcionario)
            .then(() => {
...
function carregarListaFuncionarios() {
    ...
    const ref = database.ref('FuncionariosEstoque');
    ref.once('value')
        .then(snapshot => {
...
function editarFuncionario(cpf) {
    database.ref('FuncionariosEstoque/' + cpf).once('value')
        .then(snapshot => {
...
function excluirFuncionario(cpf) {
    if (confirm('Tem certeza que deseja excluir o funcionário CPF ' + cpf + '?')) {
        database.ref('FuncionariosEstoque/' + cpf).remove()
            .then(() => {
