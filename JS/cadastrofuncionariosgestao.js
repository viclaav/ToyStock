...
document.getElementById('form').addEventListener('submit', function(e) {
    ...
    if (funcionarioId) {
        // Atualização
        database.ref('FuncionariosGestao/' + funcionarioId).update(funcionario)
            .then(() => {
                mensagem.innerText = "Funcionário atualizado com sucesso!";
                mensagem.style.color = "#bfdbf3";
        ...
        database.ref('FuncionariosGestao/' + cpf).set(funcionario)
            .then(() => {
                mensagem.innerText = "Funcionário cadastrado com sucesso! CPF: " + cpf;
                mensagem.style.color = "#bfdbf3";
                document.getElementById('form').reset();
                carregarListaFuncionarios();
            })
...
// 3. Botão de Consultar Funcionários
document.getElementById('btnConsultar').addEventListener('click', function() {
    carregarListaFuncionarios();
});

// 4. Função para carregar lista de funcionários
function carregarListaFuncionarios() {
    const lista = document.getElementById('listaFuncionarios');
    lista.style.display = 'block'; // MOSTRA a lista
    lista.innerHTML = "<p>Carregando...</p>";

    const ref = database.ref('FuncionariosGestao');
    ref.once('value')
        .then(snapshot => {
...
function editarFuncionario(cpf) {
    database.ref('FuncionariosGestao/' + cpf).once('value')
        .then(snapshot => {
            ...
            if (f) {
                ...
                document.getElementById('mensagem').style.color = "#bfdbf3";
...
function excluirFuncionario(cpf) {
    if (confirm('Tem certeza que deseja excluir o funcionário CPF ' + cpf + '?')) {
        database.ref('FuncionariosGestao/' + cpf).remove()
            .then(() => {
